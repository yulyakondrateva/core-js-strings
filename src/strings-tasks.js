...
function removeFirstOccurrences(str, value) {
  const i1 = str.indexOf(value);
  if (i1 === -1) {
    return str;
  }
  const i2 = i1 + value.length;
  const res = str.substring(0, i1);
  const res2 = str.substring(i2);
...
function removeLastOccurrences(str, value) {
  const i1 = str.lastIndexOf(value);
  if (i1 === -1) {
    return str;
  }
  const i2 = i1 + value.length;
...
function isPalindrome(str) {
  ...
  while (l < r) {
    if (!letters.includes(s[l])) {
      l += 1;
    } else if (!letters.includes(s[r])) {
      r -= 1;
    } else if (s[l] !== s[r]) {
      return false;
    } else {
